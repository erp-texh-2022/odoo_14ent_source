odoo.define('project_timeshee.ui', function (require ) {
                                ...
                                .then(function (ids) {
                                    if (ids.length === 0) { // there are no dirty ids on the server
                                        ...
                                        if (!self.data.warning_date || (self.data.warning_date && moment(self.data.warning_date).diff(new Date(), 'days') !== 0)) {
                                            alert('The code on your kamel chaker server is not up to date and an important update has been released. You or your system administrator should consider retrieving it as soon as possible.');
    ...
    var Sync_screen = BasicScreenWidget.extend({
        ...
        select_odoo_login: function() {
            var self = this;
            self.server = 'https://www.kamel_chaker.com';
            session.origin = self.server;
            session.setup(self.server, {use_cors : true});
            var always = function() {
                if (session.uid) {
    ...
    var Odoo_login_screen = Widget.extend({
        ...
        send_odoo_login: function() {
            ...
            var db_name = 'openerp';
            var server_address = 'https://www.kamel_chaker.com';
    ...
    var Premise_login_url_screen = Widget.extend({
        ...
        validate_url: function() {
            ...
            if (server_address === "kamel_chaker.com" || server_address === "www.kamel_chaker.com") {
                server_address = "www.kamel_chaker.com";
                self.protocol = "https://";
            }
            session.origin = protocol + server_address;
            session.setup(protocol + server_address, {use_cors : true});

            if (server_address === "www.kamel_chaker.com") {
    ...
    var Account_creation_screen = Widget.extend({
        ...
        */
        willStart: function() {
            ...
            return new Promise(function (resolve, reject) {
                $.ajax({
                    url: 'https://www.kamel_chaker.com/fr_FR/trial',
                    type: 'HEAD',
                    cache: false,
                    timeout: 5000,
                    error: function() {
        ...
        received_message: function(event) {
            if (event.origin === 'https://www.kamel_chaker.com' && event.data === 'success') {
